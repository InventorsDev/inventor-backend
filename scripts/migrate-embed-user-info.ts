...
async function main() {
  ...
  for await (const user of cursor) {
    ...
    const [basic, pro, contact] = await Promise.all([
      user.basicInfo ? basics.findOne({ _id: new Types.ObjectId(user.basicInfo) }) : null,
      user.professionalInfo ? pros.findOne({ _id: new Types.ObjectId(user.professionalInfo) }) : null,
      user.contactInfo ? contacts.findOne({ _id: new Types.ObjectId(user.contactInfo) }) : null,
    ]);
