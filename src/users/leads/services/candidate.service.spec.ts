...
import { LeadAssignmentPositions, LeadCandidateStatus, type LeadCandidate } from "src/shared/schema";
...
describe('LeadCandidateService', () => {
        ...
        it('should return all candidates for a session', async () => {
            ...
            const candidates: LeadCandidate[] = [
                ...
                },
                {
                    ...
                    userId: new mongoose.Types.ObjectId(),
                    sessionId,
                    status: LeadCandidateStatus.ACCEPTED,
                    recommendedBy: new mongoose.Types.ObjectId(),
                    recommendedAt: new Date(),
            ...
            const result = await service.getAllSessionCandidates(new mongoose.Types.ObjectId(sessionId));
    ...
    describe('isExistingCandidates', () => {
        it('should return true when a candidate with the given email exists in the session', async () => {
            ...
            const result = await service.isExistingCandidates('candidate1@example.com', sessId);
        ...
        it('should return false when no candidate with the given email exists in the session', async () => {
            ...
            const result = await service.isExistingCandidates('nonexistent@example.com', sessId);
